...
class UserCommand(object):
    def __init__(self, controller='keyboard', command_frequency=20, display_frequency=10):
        ...
        self.in_wave            = False
        self.in_square_in_joint = False
        self.in_square_in_cart  = False
        self.in_arms_swing      = False
        ...
        self.wave_count = 0
        self.wave_hand  = 1
        self.square_in_joint_count = 0
        self.square_in_cart_count  = 0
        self.arms_swing_count = 0
        self.start_swing_count = 0
    ...
    def update(self):
        if self.in_arms_swing:
            if self.start_swing_count < 15:
                self.start_swing_count += 1
            else:
                self.arms_swing()
        ...
        if self.in_square_in_joint:
            self.square_in_joint()
        
        if self.in_square_in_cart:
            self.square_in_cart()
    ...
    def operate(self):
        if self.controller == 'keyboard':
            ...
            if cmd == '3':
                self.log_traj = []
                self.in_square_in_joint = True
            
            if cmd == '4':
                self.log_traj = []
                self.in_square_in_cart = True
    ...
    def square_in_joint(self):
        if self.square_in_joint_count < square_trajectory_joint_space[0].size:
            for idx, joint in enumerate(ARM_JOINT_LIST):
                Bruce.joint[joint]['q_goal'] = square_trajectory_joint_space[idx][self.square_in_joint_count]
            Bruce.set_command_arm_positions()
            arm_data = MM.ARM_STATE.get()
            q  = arm_data['joint_positions']
            self.log_traj.append(arm_fk([q[0], q[1], q[2]]))
            self.square_in_joint_count += 1
        else:
            self.square_in_joint_count = 0
            self.in_square_in_joint = False
            # Plot 
...
####################################### Assignment 2 #######################################
    def lay_arms(self):
        for i in range(lay_arms_traj[0].size):
            for idx, joint in enumerate(ARM_JOINT_LIST):
                # --- TODO: Set the joint goal position to lay the arms down --- #
                pass
                # Hint: Have a look at the code above for reference
                # -------------------------------------------------------------- #
            Bruce.set_command_arm_positions()
            time.sleep(0.05)
    ...
    def reset_arms(self):
        for i in range(lay_arms_traj[0].size):
            for idx, joint in enumerate(ARM_JOINT_LIST):
                # --- TODO: Set the joint goal position to set the arm to nominal position --- #
                pass
                # Hint: You can also use lay_arms_traj
                # ---------------------------------------------------------------------------- #
            Bruce.set_command_arm_positions()
            time.sleep(0.05)
        self.arms_swing_count = 0
    ...
    def arms_swing(self):
        # --- TODO: Set the joint position to swing both arms --- #
        pass
        # Hint: You can use self.arms_swing_count as a counter
        # Think about how to make the trajectory periodic
        # ------------------------------------------------------- #
############################################################################################
